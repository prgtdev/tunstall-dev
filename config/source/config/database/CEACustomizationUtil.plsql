...
PROCEDURE Create_Supplier_Forecast_Temp_View IS
  ...
  pivot_clause          clob;  
begin
  select listagg('''' || WEEK || ''' as "' || WEEK || '"', ',') within group (order by WEEK) 
  into   pivot_clause
 ...
 dbms_output.put_line(pivot_clause);
  ...
  sql_stmt := 'CREATE OR REPLACE VIEW SUPPLIER_FORECAST_TEMP AS select *
  from (Select CF$_PART_NO,CF$_SUPPLIER_NO,WEEK,to_number(FORECAST) as forecast
          from (select *
                  from (select t.CF$_PART_NO,
                               t.CF$_SUPPLIER_NO,
                               t.CF$_WEEK_STR_01,
